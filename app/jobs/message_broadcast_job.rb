class MessageBroadcastJob < ApplicationJob
  ...
  def perform(message)
    	ActionCable.server.broadcast "room:#{message.room_id}", { my_message: render_current_user_message(message), other_message: render_other_users_message(message), room_id: message.room_id, user_id: message.user_id}
  end
